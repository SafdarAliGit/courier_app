...
def _make_barcode_html(shipment_name, barcode_type, bar_height):
    ...
    try:
        ...
        svg = generate_barcode_svg(shipment_name, barcode_type=barcode_type, bar_height=bar_height, show_text=False)
        return svg
    except Exception:
        return ""
...
.barcode-cut::before {{ margin-right:5pt; }}
.barcode-cut::after  {{ margin-left:5pt; }}
.barcode-bottom {{
  display:flex; flex-direction:column; align-items:center; justify-content:center;
  padding:10pt 10pt 6pt; background:#fafafa;
  page-break-inside:avoid;
}}
.barcode-bottom svg {{ display:block; max-width:280pt; height:auto; }}
.barcode-id {{
  font-family:'Courier New',monospace; font-size:11pt; font-weight:800;
  color:#111; margin-top:4pt; letter-spacing:0.08em; text-align:center;
}}
...
.cut-line {{
  display:flex; align-items:center; margin:20pt 0; color:#bbb;
  font-size:9pt; line-height:1;
}}
...
"""
  ...
  {(f'<div class="barcode-cut">&#9986;</div><div class="barcode-bottom">{barcode_html}<div class="barcode-id">{_esc(name)}</div></div>') if barcode_html else ''}"""
